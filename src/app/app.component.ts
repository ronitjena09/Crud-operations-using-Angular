...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    const localData = localStorage.getItem("crudoperations");
    if(localData != null) {
  ...
  onDelete(item: Person) {
    ...
    if(isDelet) {
      ...
      localStorage.setItem('crudoperations', JSON.stringify(this.PersonList));
    }
  }
  onEdit(item: Person) {
    this.PersonObj =  item;
  ...
  savePerson() {
    debugger;
    const isLocalPresent = localStorage.getItem("crudoperations");
    if (isLocalPresent != null) {
      ...
      localStorage.setItem('crudoperations', JSON.stringify(oldArray));
    } else {
      const newArr = [];
      newArr.push(this.PersonObj);
      this.PersonObj.id = 1;
      this.PersonList = newArr;
      localStorage.setItem('crudoperations', JSON.stringify(newArr));
    }
    this.closeModel()
  }
}
